    ...
    <div class="common-grid-wrapper">
        ...
        <div class="common-grid-container">
            ...
            <table class="common-grid-table">
                <thead>
                    <tr>
                        <th v-for="(i, index) in tableColumns" :key="index" class="table-head">
                            {{ i.title  }}
                        </th>
                    </tr>
                </thead>
                <tbody v-if="!isShowAppendTable">
                    <tr v-for="(i, index) in tableData" :key="index" @click="onTableClick(i)">
                        <td >{{ index }}</td>
                        <td v-for="(j, indexs) in i" :key="indexs">{{ j.v }}</td>
                    </tr>
                </tbody>
                <tbody v-else>
                    <tr v-for="(i, index) in 18" :key="index" @click="onTableClick">
                        <td >{{ index }}</td>
                        <td v-for="(j, indexs) in tableData" :key="indexs">{{ j.v }}</td>
                    </tr>
                </tbody>
            </table>
                ...
                await screenTradeItemsAPI({ params  })
                     .then(res => {
                         ...
                         this.tableData = rows.map(m => {
                             var obj = [
                                  /* no */ 
                                { k: 'product_name',    v: m.productName },
                                { k: 'specification',   v: m.specification },
                                { k: 'produce_company', v: m.producer_org },
                                { k: 'batch_number',    v: m.batch_number },
                                { k: 'quantity',        v: m.quantity },
                                { k: 'sales_company',   v: m.sale_org },
                                { k: 'apply_cel',       v: '' },
                                { k: 'deliver_time',    v: m.at },
                                { k: 'grn_time',        v: m.in_time }
                             ]
                             return obj
                         }) 
                    })
