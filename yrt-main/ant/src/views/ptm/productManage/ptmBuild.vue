    ...
    <div class="box">
        ...
        <div class="boxdown">
            <div
                ...
                style="width: 100%; padding: 2px 4px 4px 4px"
            >
                <bottom-tabs-wrapper>
                    <!-- tab切换区域开始 -->
                    <a-tabs class="align-l" style="padding-top: 0; height: calc(100% - 4px)" type="card">
                        <a-tab-pane v-for="item of tabName" :key="item.key" :tab="item.name">
                            ...
                            <template v-else>
                                ...
                                <div class="bottom-tabs-container" v-if="item.key == 'test'">
                                    <bottom-pictures :list="examImgList" @previewImg="previewImg" @delete="deleteImg" />
                                </div>
                                <!-- 其他 -->
        ...
        <a-modal
            ...
            @ok="printFn"
            @cancel="closePreview"
            class="modal-form-input-scoped global-drag img_ptm"
        >
            <div v-globalDrag="{ el: 'img_ptm' }" class="modal-header-wrap">
                <h6>
        ...
        <a-modal
            ...
            @cancel="editSt = false"
            @ok="editGoodsFn"
            class="modal-form-input-scoped global-drag edit_ptm"
        >
            <div v-globalDrag="{ el: 'edit_ptm' }" class="modal-header-wrap">
                <h6>
                    <img src="/images/logo/logo-left.svg" alt="" class="modal-logo-left" />
                    {{ leftTitle }} - 修改
                </h6>
        ...
        <a-modal
            ...
            class="modal-form-input-scoped global-drag entry_ptm"
        >
            <div v-globalDrag="{ el: 'entry_ptm' }" class="modal-header-wrap">
                <h6 v-if="mdrfIsNew">
                    <img src="/images/logo/logo-left.svg" alt="" class="modal-logo-left" />
...
export default {
    ...
    methods: {
        ...
        async loadTable(chooseObj) {
            ...
            await platformProductListAPI(0, { params }).then((res) => {
                if (parseFloat(res.code) === 0) {
        ...
        openEdit() {
            ...
            if (this.chooseTrObj.product_inner_sn) {
                // 赋值
                this.editSt = true;
                this.$Utils.globalDragCenterFn('edit_ptm');
                if (this.$refs.editPage) {
                    this.sendEditInfo();
                    this.$Utils.globalDragCenterFn('edit_ptm');
                } else {
                    this.$Utils.globalDragCenterFn('edit_ptm', true);
                }
            } else {
                this.$warning({
                    title: '操作提示',
        ...
        previewImg(src) {
            ...
            if (src && src !== '') {
                ...
                img.onload = function () {
                    ...
                    that.preImgSt = true;
                    that.imgSrc = src;
                    that.$Utils.globalDragCenterFn('img_ptm');
                };
            }
        },
        closePreview() {
        ...
        openGicMdrf() {
            ...
            if (this.$refs.mdrfGicPage) {
                this.$Utils.globalDragCenterFn('entry_ptm');
                this.$refs.mdrfGicPage.refreshPage();
            } else {
                this.$Utils.globalDragCenterFn('entry_ptm', true);
            }
        },
        async editGicMdrf() {
            // 修改注册证
        ...
        highSearch(val) {
            ...
            if (val) {
                ...
                for (let i = 0; i < box.length; i++) {
                    document.getElementsByClassName('boxtop')[i].style.height = this.boxTopHeight + 59 + 'px';
                }
            } else {
                this.boxTopHeight += 38;
                for (let i = 0; i < box.length; i++) {
