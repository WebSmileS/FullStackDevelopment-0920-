    ...
    <div class="scoped agency-approval-container">
            ...
            <a-row class="btn-top-wrap">
                ...
                <a-col :span="12">
                    ...
                    <div class="input-search-wrap">
                        <a-input-search
                            v-model="searchTables.social_credit_code"
        ...
        <div class="tab-table-wrap tab-table-overflowy">
            ...
            <a-tabs @change="changeTabsFn" type="card">
                <a-tab-pane v-for="tabItem in agencyTab" :tab="tabItem.key" :key="tabItem.systemTypes" forceRender>
                    <div class="box">
                        <div class="boxtop">
                            <vxe-table
                                ref="xTable"
                            ...
                            >
                                <vxe-table-column
                                    v-for="columnItem in tableSet[tabItem.sTypeNum - 1]"
                                    :key="columnItem.key"
                        ...
                        <div class="boxdown">
                            <!--表格区域-->
                            <!-- 下半部tab切换区域开始 -->
                            <div class="agency-down-tabs1" v-bind:style="{ height: boxDownHeight - 4 + 'px' }">
                                <!-- 表格头部 -->
                                <bottom-tabs-wrapper>
                                    <a-tabs
...
export default {
    ...
    data() {
        return {
            ...
            tableSet: TabTypeCfg.AgencyManageTableTab(),
    ...
    methods: {
        ...
        AdvancedSearch() {
            ...
            this.provinceSearchDefaults = [];
            if (this.searchStatus) {
                this.boxTopHeight -= 60;
                try {
                    document.getElementsByClassName('boxtop')[0].style.height = this.boxTopHeight + 55 + 'px';
                } catch (e) {
                    console.log(e);
                }
            } else {
                this.boxTopHeight += 60;
                try {
                    document.getElementsByClassName('boxtop')[0].style.height = this.boxTopHeight + 56 + 'px';
                } catch (e) {
                    console.log(e);
                }
            }
        ...
        // 根据注册机构类型【获取机构列表】
        async platformAgencyAuthListAPIFn(systemType = this.tabsActiveKey) {
            let sysType = 0;
            // 判断参数有没有， 没有直接不要传给后端
            let params = {
                // 排序
            ...
            await platformAgencyAuthListAPI(sysType, {
                params
            }).then((res) => {
                if (parseFloat(res.code) === 0) {
                    ...
                    if (res.rows.length !== 0) {
                        rowsNew = rows.map((item) => {
                            ...
                            item.data_source_text = this.dataSource[parseFloat(item.data_source)];
                            item.activeClass = '';
                            return item;
                        });
                    }
        ...
        async platformAddNewVendorAPIFn(params) {
            ...
            await platformAddNewVendorAPI(params).then((res) => {
                this.AgencyInfo.loading = false;
                // this.addNewVendor = false  //这句是修复第一次新增报错，再点一下就变成修改
                if (parseFloat(res.code) === 0) {
                    this.editInfoFormNewObj = {};
                    this.AgencyInfo.alert = false;
            ...
            switch (this.tabsActiveKey) {
                case 'hospital':
                    if (this.agencyTab[1].Datas.length) {
                        ...
                        this.changeDownTabsFn('1');
                        setTimeout(() => {
                            this.$refs.xTable[1].resetAll();
                            this.$refs.xTable[1].setCurrentRow(this.currentTrObj);
                        }, 10);
                    } else {
                        this.currentTrObj = {};
                    }
                ...
                case 'dealer':
                    if (this.agencyTab[0].Datas.length) {
                        ...
                        this.changeDownTabsFn('1');
                        setTimeout(() => {
                            this.$refs.xTable[0].resetAll();
                            this.$refs.xTable[0].setCurrentRow(this.currentTrObj);
                        }, 10);
                    } else {
                        this.currentTrObj = {};
                    }
                ...
                case 'vendor':
                    if (this.agencyTab[2].Datas.length) {
                        ...
                        this.changeDownTabsFn('1');
                        setTimeout(() => {
                            if (this.$refs.xTable[2]) {
                                this.$refs.xTable[2].resetAll();
                                this.$refs.xTable[2].setCurrentRow(this.currentTrObj);
                            }
                        }, 10);
                    } else {
                        this.currentTrObj = {};
        ...
        // 修改模态框操作
        AgencyInfoOkFn() {
            if (this.editInfoFormObj.establish_date == null) {
                this.editInfoFormObj.establish_date = moment[''];
                this.editInfoFormObj.establish_date = null;
            }
        ...
        changePaginationFn(page) {
            ...
            this.platformAgencyAuthListAPIFn(this.tabsActiveKey);
        },
        // 表格重新刷新数据的初始化函数 01
        initRfreshTablesFn() {
        ...
        checkSearchInfoFormMsgFn() {
            if (this.$Utils.checkInputValFn(this.searchTables.social_credit_code)) {
                this.$warning({
                    title: '操作提示',
            ...
            if (this.searchStatus && this.$Utils.checkInputValFn(this.searchTables.name)) {
                this.$warning({
                    title: '操作提示',
        ...
        previewImg(src) {
            ...
            if (src && src !== '') {
                ...
                img.onload = function () {
                    ...
                    that.imgPreviewTitle = '机构 - ' + current.tab + ' - 图片预览';
        ...
        openUploadGic() {
            ...
            this.uploadTitle = '机构 - ' + current.tab + ' - 上传图片';
