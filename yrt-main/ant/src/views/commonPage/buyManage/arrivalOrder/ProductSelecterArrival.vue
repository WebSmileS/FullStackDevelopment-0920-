        ...
        <div class="modal-form-input" style="padding-top: 0">
            ...
            <ul class="top-list" style="width: 100%">
                <li class="item">
                    ...
                    <div class="input">
                        <a-input placeholder="厂商" size="small" v-model="searchValues.productVendorName" />
                    </div>
                </li>
                <li class="item">
...
import { arrivalOrderProductListAPI } from '@/service/pageAjax';
import productColumns from './productSelecterArrivalColumns';

export default {
    name: 'ProductSelecterArrival',
    props: {
        // 业务类型
        billsType: {
            type: String,
    ...
    methods: {
        ...
        handleSubmit(products) {
            ...
            if (_products && _products.length > 0) {
                this.$emit('submit', { arrivalOrderProducts: _products });
        ...
        async listLoader(currentPage, pageSize, searchValues) {
            try {
                this.loading = true;
                const { code, msg, rows, total } = await arrivalOrderProductListAPI(currentPage, pageSize, {
