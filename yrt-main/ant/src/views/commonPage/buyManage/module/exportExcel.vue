<template>
    <div class="scoped" style="margin: -24px; min-height: 600px">
        ...
        <div class="search-item digest">
            ...
            <div class="input_box">
                {{ this.exportFormData.desc.value }}
            </div>
        </div>
...
export default {
                ...
                },
                {
                    ...
                    slots: {
                        default: ({ row }) => {
                            ...
                            // let dom;
                            // dom = [
                            //     <a-input-number min={0} v-model={record.quantity} class="input" onChange={change} />
                            // ];
                            // return dom;
                        }
                    }
    ...
    methods: {
        ...
        exportExcel() {
            ...
            const formData = {
                ...
                desc: {
                    label: '摘要',
                    value: this.exportFormData.desc.value
                },
                groups: [
                    [
                        {
                            label: '单位名称',
                            value: this.proTableData[0].vendor_name
                        },
                        {
                            label: '联系人',
                            value: this.exportFormData.contacts
                        },
                        {
                            label: '电话',
                            value: this.exportFormData.phone
                        }
                    ]
                ]
            };
    ...
    created() {
        let exportFormData = window.localStorage.getItem('exportFormData');
        exportFormData = JSON.parse(exportFormData);
        console.log(exportFormData);
        this.exportFormData = exportFormData;

        let exportProductData = window.localStorage.getItem('exportProductData');
        let products = JSON.parse(exportProductData);
        console.log(products);
        this.proTableData = products;
    }
};
</script>
