            ...
            <a-tabs @change="changeTabsFn" type="card">
                <a-tab-pane v-for="tabItem in agencyTab" :tab="tabItem.key" :key="tabItem.systemTypes">
                    <div class="box">
                        ...
                        <!-- <a-table
                            :scroll="{ y: clientHeight - 520 }"
                            :loading="tabItem.Pagination.loading"
                            :columns="tableSet[tabItem.sTypeNum - 1]"
                            :dataSource="tabItem.Datas"
                            :pagination="{
                                size: 'small',
                                current: tabItem.Pagination.current,
                                total: tabItem.Pagination.total,
...
export default {
    ...
    data() {
        return {
            ...
            agencyTab: TabTypeCfg.licDealerTabSet(),
            tableSet: TabTypeCfg.licDealerTableTAb(),
    ...
    methods: {
        ...
        // 根据注册机构类型【获取机构列表】
        async licManageDealerFn(systemType = this.tabsActiveKey) {
            if (this.tabsActiveKey === 'dealer') {
        ...
        changePaginationFn({ currentPage, pageSize }) {
            ...
            this.licManageDealerFn(this.tabsActiveKey);
        },
        // 表格重新刷新数据的初始化函数 01
        initRfreshTablesFn() {
        ...
        previewImg(src) {
            ...
            if (src && src !== '') {
                ...
                img.onload = function () {
                    ...
                    that.imgPreviewTitle = '经销商 - ' + current.tab + ' - 图片预览';
