    ...
    beforeCreate() {
        let res = {
            info: {
                ...
                system_type: this.$cookie.get('userSystemType'),
                password_cycle: this.$cookie.get('userPasswordCycle'),
                user_inner_sn: this.$cookie.get('userInnerSn')
            },
            isLogin: ''
        };
        if (this.$cookie.get('loginStatus')) {
            ...
            this.$store.commit('changeStateInfo', { info: {}, isLogin: Math.random() });
        }
