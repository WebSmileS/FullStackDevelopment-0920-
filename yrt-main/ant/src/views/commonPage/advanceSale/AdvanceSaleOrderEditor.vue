        ...
        <template v-slot:content="{ fullscreen }">
            ...
            <editor-form-header>
                <editor-form-header-item>
                    <span class="label"><span v-if="!isReadonly" class="red">*</span>采购方：</span>
    ...
    methods: {
        ...
        onProductAdd(data) {
            ...
            this.$nextTick(() => {
                ...
                // 从尾部遍历列表，自动给新添加的产品填充批号
                for (let i = list.length - 1; i >= 0; i -= 1) {
                    if (list[i].batchNumberLoaded === false) {
                        this.handleAutoSetRowBatchNumber(list[i]);
                        delete list[i].batchNumberLoaded;
                    } else {
                        break;
                    }
                }
         ...
         * @param {string} row._XID 行数据的id标识，此标识符默认为vex-table自动生成，批号的更新依赖于此标识
         */
        async handleAutoSetRowBatchNumber(row) {
            try {
                const orgId = this.$cookie.get('userbelong');
                const { code, msg, list } = await odoBatchListAPI({
                    org_id: orgId,
                    billsType: this.dataEditing.billsType,
                    warehouse_inner_sn: this.dataEditing.warehouseId,
                    product_inner_sn: row.productId,
                    product_type: row.productType,
                    product_vendor_inner_sn: row.productVendorId,
                    specification_inner_sn: row.specificationId
                });
                const details = this.dataEditingDetails;
                if (code !== 0 || !list[0]) {
                    throw new Error(msg);
                }
                for (let i = details.length - 1; i >= 0; i -= 1) {
                    if (row._XID === details[i]._XID) {
                        row.batchNumber = list[0].batch_number;
                        break;
                    }
                }
            } catch (err) {
                this.$message.warning(`产品：${row.productName} 的批号自动填写失败，请手动填写`);
            }
        },
        ...
        async loadOrderInfo(id) {
            try {
                ...
                if (code !== 0) {
                    this.$message.error(msg || '预售单详情加载失败');
                    throw new Error(msg);
                }
        ...
        async addOrder(data) {
            try {
                ...
                if (code !== 0) {
                    this.$message.error(msg || '预售单创建失败');
                    throw new Error(msg);
                }
                this.$message.success('预售单创建成功');
                return info;
            } finally {
        ...
        async editOrder(id, data) {
            try {
                ...
                if (code !== 0) {
                    this.$message.error(msg || '预售单保存失败');
                    throw new Error(msg);
                }
                this.$message.success('预售单保存成功');
                return info;
            } finally {
                this.buttonsLoading.save = false;
            }
