<template>
    <list-page-layout>
        ...
        <template slot="list">
            <invoice-search-list
                ref="list"
                :listType="invoiceSearchListMap.ODO.value"
...
import { store as invoiceSearchStore, STORE_NAME as invoiceSearchStoreName, SET_ODO_INVOICE_INFO } from './store';
    ...
    methods: {
        ...
        handleCreateInvoice() {
            ...
            if (productsFiltered.length) {
                this.$confirm({
                    title: '不能同时开具多个“单位名称”的出库发票',
                    content: `已选择的产品（共${this.selected.length}个）中包含不属于机构“${this.targetOrganization.organizationName}”的出库产品（共${productsFiltered.length}个），将会被自动过滤，确定继续吗？`,
                    onOk: () => {
                        this.handleCreateODOInvoice(this.targetOrganization, products);
                    }
                });
            } else {
                this.handleCreateODOInvoice(this.targetOrganization, products);
         ...
         */
        handleCreateODOInvoice(organization, products) {
            this.$store.commit(`${invoiceSearchStoreName}/${SET_ODO_INVOICE_INFO}`, {
                products,
                billsType: organization.billsType,
                organizationInnerSn: organization.organizationInnerSn,
                organizationName: organization.organizationName
            });
            this.$router.push({ name: 'outboundSystemInvoice' });
        }
    }
};
</script>
