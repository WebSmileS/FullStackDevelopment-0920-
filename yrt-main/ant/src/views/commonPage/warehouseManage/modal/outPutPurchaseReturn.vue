    ...
    <div class="scoped scoped-white page-warehousemanage-outputstorage">
        <div style="height: 21px">
            <a-breadcrumb style="float: left">
                <a-breadcrumb-item style="color: #1890ff">计划</a-breadcrumb-item>
                <a-breadcrumb-item>新增</a-breadcrumb-item>
            </a-breadcrumb>
        </div>
        <a-row class="shop-card">
            <a-row class="shop-card-header">
                <div class="even-columns group">
                    <div class="col">
                        <div class="col-label"
                            ><label class="form-label"><em class="em red">*</em>&nbsp;{{ companyName }}</label></div
                        >
                        <div class="col-input">
                            <!-- <a-input
                    ...
                    <div class="col">
                        <div class="col-label"
                            ><label class="form-label"><em class="em red">*</em>&nbsp;仓库</label></div
                        >
                        <div class="col-input">
                            <a-tree-select
                                ref="warehouseFocus"
                    ...
                    <div class="col">
                        <div class="col-label"
                            ><label class="form-label"><em class="em red">*</em>&nbsp;部门</label></div
                        >
                        <div class="col-input">
                            <a-tree-select
                                treeNodeFilterProp="title"
            ...
            <a-row class="shop-card-content">
                ...
                >
                    ...
                    <vxe-table-column field="index" title="NO." width="50" :resizable="false" fixed="left">
                        <template v-slot="{ row, seq }">
                            <div>
                                <i
                                    :class="row.source_type === 'copy' || !row.detail_inner_sn ? 'red-dot-add' : 'hide'"
                                ></i>
                                <span>{{ seq }}</span>
                            </div>
                        </template>
                    ...
                    <vxe-table-column :visible="isShowGiveOdoColumn" field="unit_price" title="含税单价" width="80">
                        <template v-slot="{ row, seq }">
                            <span v-if="!row.product_inner_sn"></span>
                            <a-dropdown v-else :disabled="row.isReadonly || isEditDisabledAddInfo" :trigger="['click']">
                                <a-input-number
                                    :min="0"
                            ...
                            <!-- <a-input-number
                                ...
                                class="input"
                                style="width:100%"
                                @change="updataTable(row, 'unit_price', seq)"
                            /> -->
                        </template>
                    </vxe-table-column>
                    <!-- 含税金额 重写 -->
                    ...
                    <vxe-table-column v-if="isShowAddInfoModalActionColumn" field="action" title="操作" width="100">
                        <template v-slot="{ row, seq }">
                            ...
                            <!-- 其它情况 -->
                            <div
                                v-else-if="row.product_inner_sn && (btnName === 'add' || addInfoModalForm.status === 0)"
                            >
                                <ul class="action-btn-list-wrap" style="position: relative">
                                    <li>
                ...
                <a-row class="card-add-row">
                    <a v-on:click="popupAddProduct">添加产品</a>
                </a-row>
                <a-row>
                    <ul class="order-modal-list">
                        <li>
                            <div
                                ...
                                v-if="
                                    !c_IsGrn &&
                                    (addInfoModalForm.billsType === 'DH' || addInfoModalForm.billsType === 'VH')
                                "
                            >
                ...
                <a-col :sm="22" :md="22" :lg="18" :xl="18" :xxl="15">
                    <ul class="btn-list-wrap">
                        ...
                        <li>
                            <!-- 已审核状态的单据才能 [入/出库] -->
                            <a-button
                                v-if="!this.isTableDatasEmpty && currentTrObj.status === 1"
                            ...
                            <a-button
                                v-if="!this.isTableDatasEmpty && currentTrObj.status === 4"
            ...
            <a-row :gutter="16" v-show="hightSearchObj.isArrow" class="hight-search-wrap">
                ...
                <a-col :span="6">
                    ...
                    <a-tree-select
                        ...
                        class="form-input"
                        size="small"
                        v-model="searchTables.warehouse_model_inner_sn"
            ...
            <!-- <a-row :gutter="16" style="margin-top: 20px;">
                ...
                <a-col :span="6">
                    ...
                    <a-select
                        ...
                        allowClear
                        class="form-input"
                        style="width: 70%; height: 32px; display: inline-block; vertical-align: middle;"
                    >
                        <a-select-option  class="input" value="0" key="0">正式</a-select-option>
                        <a-select-option  class="input" value="1" key="1">已审核</a-select-option>
                        <a-select-option  class="input" value="2" key="2">已入库</a-select-option>
        ...
        >
            <!-- <a-tabs type="card"
                    :activeKey="tabChoose"
                    @change="changeTabFn"> -->
            <!-- <a-tab-pane :tab="tabTypes[0]" :key="tabTypesNum[0]" style="padding: 8px 8px 0px 8px" > -->
            <vxe-table
                ref="xTable"
                size="small"
            ...
            <!-- </a-tab-pane> -->
            <!-- <a-tab-pane :tab="tabTypes[1]" defaultActiveKey="0"  :key="tabTypesNum[1]" style="padding: 8px 8px 0px 8px">
                <vxe-table
                    ref="xTableSto"
                    size="small"
                    :height="scrollYNumberSto - 2"
                    border
                    resizable
                    auto-resize
                    highlight-current-row
                    @cell-click="cellClickFnSto"
                    @cell-dblclick="cellDblclickFnSto"
                    align="center"
                    :data="stoEntryDatas"
                    :loading="stoEntryPagination.loading"
                    show-overflow
                    show-header-overflow
                    showHeader
                    :start-index="(stoEntryPagination.current - 1) * stoEntryPagination.pageSize"
                >
                    <vxe-table-column
                        v-for="godownEntryColumnItem in stoEntryColumns"
                        :key="godownEntryColumnItem.key"
                        :field="godownEntryColumnItem.dataIndex"
                        :title="godownEntryColumnItem.title"
                        :width="godownEntryColumnItem.width"
                ...
                <vxe-pager
                    :loading="stoEntryPagination.loading"
                    :current-page="stoEntryPagination.current"
                    :page-size="stoEntryPagination.pageSize"
                    :page-sizes="stoEntryPagination.pageSizes"
                    :total="stoEntryPagination.total"
                    :layouts="['PrevPage', 'JumpNumber', 'NextPage', 'FullJump', 'Sizes', 'Total']"
                    @page-change="changePaginationFn">
                </vxe-pager>
            </a-tab-pane> -->
...
export default {
    ...
    methods: {
        ...
        chooseOrgFn(resObj) {
            ...
            this.selectedOrg = resObj;
            this.searchOrgName = resObj.org_name;
        ...
        searchOrg(value = '') {
            ...
            this.grnOrgListAPIFn(1);
            this.grnOrgListAPIFn(3);
            this.grnOrgListAPIFn(2);
        },
        // 【相关机构列表】 【仓库管理-入库单】
        async grnOrgListAPIFn(type) {
            let params = {};
            params.pageSize = 5;
            params.pageNum = 1;
            if (this.searchOrgName && this.searchOrgName.length) {
                params.name = this.searchOrgName;
            }
            params.all = true;
            await grnOrgListAPI(type, this.userBelong, { params: params })
                .then((res) => {
                    if (parseFloat(res.code) === 0) {
        ...
        changeApplyOrg(value) {
            if (value === 'search-more') {
                ...
                this.searchOrgName = this.selectedOrg.org_name;
                this.setOrganizationNameFn(this.selectedOrg);
            }
        },
        // ****************************************************************
        // 接口区域结束
