    ...
    <div class="and-hottable-acts">
        ...
        <div class="and-ha-dropdown" v-if="dropdownVisible">
            ...
            <div class="and-had-group">
                <a-checkbox-group @change="onChange" :default-value="headers.length > 0 ? headers : allHeaders">
                    <div
                        v-for="(d, idx) in data" 
                        :key="idx"
    ...
    methods: {
        ...
        store() {
            ...
            this.data.forEach((item, idx) => {
                if (self.headers.includes(item.data)) {
                    headers.push(item.data);
                    widths.push(self.widths[idx]);
                }
            });
            let data = {
                headers: headers
            };
            // if (this.storeFixedWidths && this.widths.length > 0) {
            ...
            if (pageName && curTab) {
                ...
                if (cookieData !== null) {
                    if (cookieData.headers) {
                        this.headers = cookieData.headers;
                    }
                }
            }
        }
