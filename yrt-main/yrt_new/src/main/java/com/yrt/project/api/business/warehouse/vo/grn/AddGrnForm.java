...
@Data
public class AddGrnForm implements Serializable {
     ...
     * 类型内部编号
     */
    @ApiModelProperty(value = "入库单类型ID", required = true)
    @NotNull(message = "入库单类型ID不能为空")
    private Long type_inner_sn;
    @ApiModelProperty(value = "入库单类型名称", required = true)
    @NotBlank(message = "入库单类型名称不能为空")
