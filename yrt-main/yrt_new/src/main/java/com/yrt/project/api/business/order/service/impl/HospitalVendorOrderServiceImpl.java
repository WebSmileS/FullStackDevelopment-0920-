...
@Service
public class HospitalVendorOrderServiceImpl implements IHospitalVendorOrderService {

    
	@Autowired HospitalVendorOrderMapper mapper;
	...
	public PurchaseOrder addPurchaseOrder(AddPurchaseOrderForm form, int system_type1, int system_type2) {
		...
		mapper.hv_add_hospital_po_vendor_so(params);
		if(Integer.valueOf(params.get("result_o").toString()) == 0) {
			...
			for (OrderDetailForm detail : details) {
				...
				mapper.hv_add_hospital_po_vendor_so_product_detail(params1);
