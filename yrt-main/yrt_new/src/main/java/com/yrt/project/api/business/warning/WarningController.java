package com.yrt.project.api.business.warning;
...
@RestController
@RequestMapping("/v1/business/warning")
@Api(tags = "业务-效期预警接口")
public class WarningController  extends BaseController {
	...
	@ApiOperation(value = "当前机构获取预警设置", notes = "获取预警设置")
	...
	public AjaxResult saveWarningSetup(
		...
		paramService.saveParameter(curUser.getSystemType(), curUser.getOrgId(), SysParameterService.WARNING, "预警设置", setupValue);
		return success();
	}
	...
	@ApiOperation(value = "当前机构及合作机构资证效期预警(分页)", notes = "当前机构及合作机构资证效期预警(分页)")
	...
	@ApiOperation(value = "当前机构有效合同内产品注册证效期预警(分页)", notes = "当前机构有效合同内产品注册证效期预警(分页)")
	...
	@SuppressWarnings("unchecked")
	@Log(title = "business.warning", businessType = BusinessType.OTHER, operatorType = OperatorType.BUSINESS)
	@RequestMapping(value = "/validity/list", method = RequestMethod.GET)
	@ApiOperation(value = "当前机构库存产品效期预警(分页)", notes = "当前机构库存产品效期预警(分页)")
	public TableDataInfo<ProductValidityWarning> getProductValidityWarningList(
			@Validated SearchWarningForm form) {
		ContextUserInfo curUser = ServletUtils.getCurUserInfo();
		startPage();
		return getDataTable(service.getProductValidityWarningList(form, curUser));
	}
