...
@Data
public class EditOdoForm implements Serializable {
	...
	/** 类型内部编号 */
	@ApiModelProperty(value="出库单类型ID",required=false)
	private Long type_inner_sn;
	private Long type_inner_sn_old;
	@ApiModelProperty(value="出库单类型名称",required=false)
	...
	public Map<String,Object> getInfo(){
		...
		if(StringUtils.isNotNull(type_inner_sn) || StringUtils.isNotNull(type_inner_sn_old)) {
			if(StringUtils.isNull(type_inner_sn)) {
				throw new ValidateException(MessageUtils.message("param.not.null", "出库单类型ID"));
			}
