<template>
    <div class="scoped single-tb" style="min-height: 600px">
        ...
        <a-modal
            ...
            :centered="true"
            :maskClosable="false"
            class="modal-form-input-scoped global-drag edit pack-unit-edit"
        >
            <div v-globalDrag="{ el: 'pack-unit-edit' }" class="modal-header-wrap">
                <h6>
...
export default {
    ...
    methods: {
        ...
        async platformUnitAddFn() {
            ...
            this.$emit('unitChanged', params.vendor_inner_sn, params.product_inner_sn);
        },
        // 【编辑】商品自定义分类
        ...
        checkEditInfoFormMsgFn() {
            ...
            let v4 = /^([1-9]\d{0,19}|0)(\.\d{1,4})?$/;
            if (!v4.test(this.editInfoFormObj.measure)) {
                this.$warning({
                    title: '操作提示',
                    content: '最小计量/倍数格式错误，整数最多20位',
                    okText: '知道了',
                    centered: true,
                    maskClosable: false
                });
