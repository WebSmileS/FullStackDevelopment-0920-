            ...
            <div class="modal-form-input scoped">
                <div class="tab-table-wrap tab-table-overflowy">
                    ...
                    >
                        ...
                        <vxe-table-column field="average_price" title="成本单价" width="20%"></vxe-table-column>
                    </vxe-table>
                </div>
            </div>
        </a-modal>
    ...
    methods: {
        ...
        async batchRequestFn() {
            ...
            let params = {
                ...
                product_vendor_inner_sn: this.table.currentSelectedRow.vd_inner_sn,
                specification_inner_sn: this.table.currentSelectedRow.specification_inner_sn,
                billsType: billsType,
                // 调拨单，机构为经销商和厂商时，可以查询到过期产品
                isReturn: this.isTransfer && (userBelong === 2 || userBelong === 3)
            };
        ...
        BatchTableClickFn({ row }) {
            ...
            this.table.tableData[index].produce_date = row.produce_date && moment(row.produce_date);
            this.table.tableData[index].overdue_date = row.overdue_date && moment(row.overdue_date);
            this.table.tableData[index].buying_price = row.average_price;
            this.table.tableData[index].quantityMax = row.quantity;
