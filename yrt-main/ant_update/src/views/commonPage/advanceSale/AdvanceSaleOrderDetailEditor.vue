            ...
            <div class="detail-table-wrapper" style="padding-top: 24px">
                <vxe-table
                    ...
                    :edit-rules="{
                        ...
                        salePrice: [{ min: 0, type: 'number', message: '含税单价不能为负' }],
    ...
    computed: {
        ...
        columns() {
            ...
            if (this.submitType === countSelecterTypeMap.RETURN) {
                return columns.filter((column) => ['rate', 'salePrice', 'money'].indexOf(column.field) < 0);
            }
            return columns;
        },
        title() {
        ...
        onEditClosed({ column, row }) {
            switch (column.property) {
                case 'editingQuantity':
                    ...
                    if (this.submitType === countSelecterTypeMap.ODO) {
                        if (row.salePrice) {
                            row.money = new Big(row.editingQuantity).times(row.salePrice).round(4).toNumber();
                            break;
                        }
                        if (row.money) {
                            row.salePrice = new Big(row.money).div(row.editingQuantity).round(4).toNumber();
                        }
                    }
                    break;
                case 'salePrice':
                    row.salePrice = new Big(row.salePrice || 0).round(4).toNumber();
                    if (this.submitType === countSelecterTypeMap.ODO) {
                        if (row.editingQuantity) {
                            row.money = new Big(row.salePrice).times(row.editingQuantity).round(4).toNumber();
                            break;
                        }
                        if (row.money) {
                            row.editingQuantity = new Big(row.money).div(row.salePrice).round(4).toNumber();
                            break;
                        }
                    }
                    break;
            ...
            for (let i = 0, l = list.length; i < l; i += 1) {
                ...
                if (!list[i].editingQuantity || list[i].editingQuantity < 0) {
                    this.$message.warning(
                        `NO.${i + 1}${list[i].productName} ${quantityTextMap[this.submitType]}不能小于零`
                    );
                    return false;
                }
                ...
                // 价格校验
                if (this.submitType === countSelecterTypeMap.ODO) {
                    if (isNaN(Number(list[i].salePrice)) || Number(list[i].salePrice) < 0) {
