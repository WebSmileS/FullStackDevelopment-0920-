    ...
    <div class="modal-tabs-pg">
        <a-tabs :activeKey="activeTab" @change="changeTabsFn" type="card">
            <a-tab-pane v-for="item of tabName" :key="item.key" :tab="item.name">
                <div v-show="item.key == 'basic'" class="modal-form-input modal-list">
                    <ul>
                        ...
                        <li class="item">
                            ...
                            <a-input :disabled="true" class="input" v-model="formData.contract_sn" />
                        </li>
                        <li class="item">
                        ...
                        <li class="item">
                            ...
                            <div class="item-children" style="width: 65%">
                                <span class="label" style="margin-left: 24px">录入人：</span>{{ formData.inputer_name }}
                            </div>
                        </li>
                    </ul>
                </div>
                ...
                <div v-if="item.key == 'product'" class="product_box scoped">
                    <!-- <div class="btn_count"> -->
        ...
        <a-modal
            ...
            class="modal-form-input-scoped global-drag add_product_ed"
        >
            <div v-globalDrag="{ el: 'add_product_ed' }" class="modal-header-wrap">
                <h6>
        ...
        <a-modal
            ...
            class="modal-form-input-scoped global-drag entryEditModal"
        >
            <div v-globalDrag="{ el: 'entryEditModal' }" class="modal-header-wrap">
                <h6>
                    <img src="/images/logo/logo-left.svg" alt="" class="modal-logo-left" />
                    医疗器械注册证（登记表） - 查看
                </h6>
...
export default {
    name: 'editDraft',
    props: ['isCloseCreateDraft', 'partType', 'editObj'],
    components: {
        chooseOrgTab,
        addProduct,
        entryGic
    ...
    data() {
        return {
            ...
            userSystemType: '', // 当前机构
            chooseOrgMd: {
                title: `${this.$route.meta.title} - 新增 - 选择机构`,
                alert: false,
                loading: false
            },
            addProductMd: {
                title: `${this.$route.meta.title} - 新增 - 相关产品 - 添加`,
                alert: false,
                loading: false
            },
            departList: [],
            ...
            formData: {
                ...
                employee_name: '', // 销售代表内部编号
                hospital_dept_inner_sn: '', // 销售区域
                sign_date: null, // 签订日期
                content: '' // 内容
            },
            oldFormData: {},
            ...
            proColumns: [
                ...
                },
                {
                    ...
                    slots: {
                        ...
                        //             // record.isNew = false
                        //             let newdata = JSON.parse(JSON.stringify(this.proTableData))
                        ...
                        //                         this.dearUnitTreeFn(record, 'edit')
    ...
    methods: {
                ...
                if (parseFloat(res.code) === 0) {
                    ...
                    if (this.part_a_obj.systemType === 1) {
                        ...
                        if (res.info.hospital_dept_inner_sn) {
                            ...
                            this.department_inner_sn = art2.join();
                        }
                    } else {
                        this.saleArea = false;
                    }
        ...
        returnData() {
            ...
            let conType = oldObj.contract_type;
            let conId = oldObj.contract_inner_sn;
            let dataObj = {};
            let baseInfoChanged = false; // 是否有修改的操作

            // 排除附件列表和产品列表比较
            const newObjTemp = this.$XEUtils.omit(newObj, ['details', 'attachmentList', 'attachments']);
            ...
            if (dataObj.payment_period_old) {
                if (dataObj.payment_period === '' || !dataObj.payment_period) {
                    this.$warning({
                        title: '操作提示',
                        content: '请输入回款周期',
                        okText: '知道了',
            ...
            dataObj.employee_name = newObj.employee_name;
            ...
            //     if (item.employee_inner_sn === dataObj.employee_inner_sn) {
            ...
            //     return false
            // }
            const oldInner = oldObj.hospital_dept_inner_sn || '';
            if (oldInner !== this.department_inner_sn) {
                dataObj.hospital_department_inner_sn = this.department_inner_sn;
                dataObj.hospital_department_inner_sn_old = oldInner;
                baseInfoChanged = true;
            }
            // 根据甲乙双方返回不同的Obj并请求不同接口
            dataObj.contract_type = conType;
            dataObj.contract_inner_sn = conId;
            ...
            for (let i = 0; i < dataObj.productList.length; i++) {
                let item = dataObj.productList[i];
                for (let j = 0; j < dataObj.productList.length; j++) {
                    let nowitem = dataObj.productList[j];
                    if (item.specification_name === nowitem.specification_name && item.key !== nowitem.key) {
                        if (
                            ...
                            this.$warning({
                                title: '操作提示',
                                content: `【No.${i + 1} - ${item.product_name}】和【No.${j + 1} - ${
                                    nowitem.product_name
                                }】添加的【产品包装单位：${item.unit_name}】选择不能相同`,
                                okText: '知道了',
                                centered: true,
                                maskClosable: true
                            });
            ...
            delete dataObj.attachments_old;
            delete dataObj.details;
            delete dataObj.details_old;

            // 对比相关产品列表和附件列表 没有改动就不做修改提交
            const productListChanged = this.$Utils.checkListChanged(dataObj.productList, oldObj.details);
            const attachmentListChanged = this.$Utils.checkListChanged(dataObj.attachmentList, oldObj.attachments);
            const changed = baseInfoChanged || productListChanged || attachmentListChanged;
            this.$emit('returnData', dataObj, changed);
        },
        changeTabsFn(val) {
            // tab切换
            if (val === 'product') {
        ...
        cellClickFn(row) {
            ...
            if (xTable && xTable[0]) {
                xTable[0].focus();
                xTable[0].clearCurrentRow();
                xTable[0].setCurrentRow(obj);
            }
        },
        cellDblclickFn(row) {
            // 双击行
        ...
        async dearUnitTreeFn(record, type) {
            // 经销商产品单位
            let oid = record.dealer_inner_sn || record.vendor_inner_sn;
        ...
        openAddPro() {
            ...
            this.$Utils.globalDragCenterFn('add_product_ed');
        ...
        uploadChange(info) {
            ...
            this.fileList = fileList.map((item) => ({
                ...item,
                name: this.$Utils.formatFileNameToLowerCase(item.name)
            }));
            ...
            await platformProductMdrfNewestListAPI(vid, pid).then((res) => {
                if (parseFloat(res.code) === 0) {
                    if (res.info) {
                        ...
                        this.$Utils.globalDragCenterFn('entryEditModal');
        ...
        keybordDirectionProductedFn(key = 'name', keyCode) {
            ...
            if (this.activeTab === 'product') {
                if (this.proTableData && this.proTableData.length) {
                    const xTable = this.$refs.relatedProductXTable;
                    if (xTable && xTable[0]) {
                        let tempRow = xTable[0].getCurrentRecord();
                        if (keyCode === 39 && (!tempRow || !tempRow.product_inner_sn)) return;
                        let index = xTable[0].getRowIndex(tempRow);
                        if (keyCode === 38) {
                            if (index <= 0) return;
                            index -= 1;
                        } else if (keyCode === 40) {
        ...
        setScrollTopFn() {
            const xTable = this.$refs.relatedProductXTable;
            if (xTable && xTable[0]) {
                const row = xTable[0].getCurrentRecord(),
                    rowHeight = 38;
                xTable[0].scrollTo(0, xTable[0].getRowIndex(row) * rowHeight);
            }
        },
        insertFocusRowFn(index = 0, keyCode) {
            ...
            const xTable = this.$refs.relatedProductXTable;
            if (xTable && xTable[0]) {
                xTable[0].refreshColumn();
                this.setScrollTopFn();
                xTable[0].setCurrentRow(this.proTableData[index]).then(() => {
