...
const store = new Vuex.Store({
    state: {
        userInfo: {
            ...
            passwordCycle: undefined,
            userInnerSn: undefined
        }, // 登录信息
        isLogin: '', // 是否已登录
        menuURI: '', // 左侧菜单的URI
        userMenu: [],
    ...
    mutations: {
        ...
        setDepartmentsOfUserOrganization(state, data) {
            if (!data || typeof data !== 'object' || data.constructor !== Array) {
                state.departmentsOfUserOrganization = null;
                return;
            }
            const treeArray = [];
        ...
        setEmployeesOfUserOrganization(state, data) {
            if (!data || typeof data !== 'object' || data.constructor !== Array) {
                state.employeesOfUserOrganization = null;
                return;
            }
    ...
    actions: {
         ...
         * 获取当前登录用户机构下的部门
         */
        async loadDepartmentsOfUserOrganization({ commit, state }) {
            const { systemType, belong } = state.userInfo;
         ...
         * 获取当前登录用户的仓库数据
         */
        async loadUserWarehouseList({ commit }) {
            const { code, msg, list } = await userWarehouseListAPI();
            if (code !== 0) {
                throw new Error(msg);
            }
