...
<template>
    <div class="product-page">
        ...
        >
            <template slot="footer">
                <a-button @click="progress.visible = false" :loading="progress.saving || progress.syncing"
                    >确认</a-button
                >
            </template>
            <div>
                ...
                ></a-progress>
                <p v-for="msg in progress.msgList" style="margin-bottom: 0.3em; text-align: left" :key="msg.key">{{
                    msg.text
                }}</p>
            </div>
        </a-modal>
...
const progressTextMap = {
    [progressMap.SAVING]: '保存中...',
    [progressMap.SAVED]: '保存成功',
    [progressMap.SYNCING]: '同步信息中...',
    [progressMap.SYNCED]: '同步信息成功'
};
...
export default {
    ...
    data() {
        return {
            ...
            progress: {
                visible: false,
                step: undefined,
                status: progressStatusMap.NORMAL,
                percent: 0,
                msgList: []
            }
        };
    ...
    methods: {
        onPackUnitChanged(vid, pid) {
            // 若产品为有效产品，自动进行信息同步
            if (
                this.chooseTrObj &&
                this.chooseTrObj.status === 1 &&
                this.chooseTrSpecObj &&
                this.chooseTrSpecObj.status === 1
            ) {
                this.handleSync(vid, pid, true);
            }
        },
        ...
        async editProServe(data) {
            try {
                const status =
                    data.status === undefined || data.status === null ? this.chooseTrObj.status : data.status;
                let vid = this.chooseTrObj.vendor_inner_sn;
                let pid = this.chooseTrObj.product_inner_sn;

                // progress
                this.progress.visible = true;
                this.progress.step = progressMap.SAVING;
                this.progress.status = progressMap.ACTIVE;
                this.progress.percent = 25;
                this.progress.msgList = [{ key: progressMap.SAVING, text: progressTextMap.SAVING }];

                const { code, msg } = await platformProductEditAPI(vid, pid, data);
                if (code !== 0) {
                    throw new Error(msg);
                }
                this.editSt = false;
                ...
                // progress
                this.progress.step = progressMap.SAVED;
                this.progress.percent = 65;
                this.progress.msgList.push({ key: progressMap.SAVED, text: progressTextMap.SAVED });

                // 若产品为无效产品，不进行信息同步
                if (status !== 1) {
                    this.progress.status = progressStatusMap.SUCCESS;
                    this.progress.percent = 100;
                    return;
                }
                await this.handleSync(data.vendor_inner_sn, data.product_inner_sn, false);
            } catch (err) {
                this.progress.status = progressStatusMap.ERROR;
                throw err;
            }
        },
            ...
            if (!this.isEdit) {
                ...
                await platformProductSpecAdd(params).then((res) => {
                    if (parseFloat(res.code) === 0) {
                        ...
                        // 有效产品，且规格信息有效时自动进行信息同步
                        if (this.chooseTrObj.status === 1 && status === 1) {
                            this.handleSync(vid, pid);
                        }
                    } else {
                        this.$message.error(res.msg);
                    }
        ...
        onSyncButtonClick() {
            ...
            this.handleSync(this.chooseTrObj.vendor_inner_sn, this.chooseTrObj.product_inner_sn, true);
        },
         ...
         * @param vid 厂商id
         * @param pid 产品id
         * @param {boolean=true} initProgress 是否初始化进度
         */
        async handleSync(vid, pid, initProgress = true) {
            try {
                // progress
                if (initProgress) {
                    this.progress.visible = true;
                    this.progress.status = progressStatusMap.ACTIVE;
                    this.progress.percent = 25;
                    this.progress.msgList = [];
                }
                this.progress.step = progressMap.SYNCING;
                this.progress.msgList.push({ key: progressMap.SYNCING, text: progressTextMap.SYNCING });

                const { code, msg } = await platformProductSyncAPI(vid, pid);
                if (code !== 0) {
                    throw new Error(msg);
                }

                // progress
                this.progress.step = progressMap.SYNCED;
                this.progress.percent = 100;
                this.progress.status = progressStatusMap.SUCCESS;
                this.progress.msgList.push({ key: progressMap.SYNCED, text: progressTextMap.SYNCED });
            } catch (err) {
                this.progress.status = progressStatusMap.ERROR;
                throw err;
            }
    ...
    watch: {
        ...
        'progress.visible': function (newValue) {
            if (!newValue) {
                this.progress.step = undefined;
                this.progress.percent = 0;
                this.progress.status = progressStatusMap.NORMAL;
                this.progress.msgList = [];
            }
        }
    },
