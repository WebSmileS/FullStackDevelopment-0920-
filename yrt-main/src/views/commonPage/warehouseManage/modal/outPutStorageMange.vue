<template>
    <div class="scoped scoped-white page-warehousemanage-outputstorage">
        <!--头部区域开始-->
        <div class="search-form-wrap">
            <a-row class="btn-top-wrap">
                <a-col :sm="22" :md="22" :lg="18" :xl="18" :xxl="15">
                    <ul class="btn-list-wrap">
                        <li v-if="orderTypeData.value !== 'GRANTODO'">
                            <a-button
                                v-if="isNotAllot || !isSeparateBillOrder"
                            ...
                            <a-button
                                v-if="isShowTransferRecordBtn"
                                @click="groupBtnFn('bill-lading')"
                                type="primary"
                                size="small"
                            >
                                调单
                            ...
                            <a-button
                                v-if="!this.isTableDatasEmpty && currentTrObj.status === 0"
                        ...
                        <li>
                            ...
                            <a-button
                                v-if="
                                    orderTypeData.value !== 'GRANTODO' &&
                                    !this.isTableDatasEmpty &&
                                    currentTrObj.status === 1
                                "
                                @click="groupBtnFn('grnEntry')"
                                type="primary"
                                size="small"
                            >
                                {{ setOutPutButtonTextComputed }}
                            </a-button>
                            <!-- 作废状态且不是调单过来的单据才能 [复制]   注：该字段voucher_type_c的值有可能为'NONE'，有可能没有值，有可能为null，因该字段关联表较多，在外层list取不到该值，必须调用对应的infoAPI才能取到，故该判断放在点击的那一瞬间 -->
                            <a-button
                                v-if="
                                    orderTypeData.value !== 'GRANTODO' &&
                                    !this.isTableDatasEmpty &&
                                    currentTrObj.status === 4
                                "
                                :loading="isCopyLoading"
                                :disabled="isCopyLoading"
                                @click="groupBtnFn('copy')"
                                type="primary"
                                size="small"
                            >
                                复制
                            ...
                            <a-button
                                v-if="
                                    ...
                                    currentTrObj.status === 1
                                "
                                @click="groupBtnFn('audit-feedback')"
                                type="primary"
                                size="small"
                            >
                                取消审核
                            ...
                            <a-button
                                v-if="
                                    !this.isTableDatasEmpty &&
                                    currentTrObj.status === 2 &&
                                    currentTrObj.type_inner_sn !== 2
                                "
                                @click="groupBtnFn('generate-red-dashed')"
                                type="danger"
                                size="small"
                            >
                                生成红冲
                ...
                <a-col :sm="4" :md="2" :lg="6" :xl="6" :xxl="9">
                    ...
                    <div class="input-search-wrap">
                        <a-input-search
                            v-show="c_IsGrn && !hightSearchObj.isArrow"
                            v-model="searchTables.grn_odo_sn"
                            placeholder="单据编号"
                            size="small"
                            enterButton
                            class="search"
            ...
            <a-row :gutter="16" v-show="hightSearchObj.isArrow" class="hight-search-wrap">
                <a-col :span="6" v-if="btnName !== 'add'">
                ...
                <a-col :span="6">
                    <span class="form-label">{{ companyName }}</span>
                    <a-input v-model="searchTables.key" type="text" placeholder="" style="width: 70%; height: 24px" />
                </a-col>
                <a-col :span="6" v-if="tabChoose !== tabTypesNum[1]">
                    <span class="form-label">产品</span>
                    <a-input
                        v-model="searchTables.product_name"
                        type="text"
                        placeholder=""
                        style="width: 70%; height: 24px"
                    />
                ...
                <a-col :span="6">
                    ...
                    <a-tree-select
                        ...
                        size="small"
                        :value="
                            typeof searchTables.warehouse_model_inner_sn === 'number'
                                ? String(searchTables.warehouse_model_inner_sn)
                                : searchTables.warehouse_model_inner_sn
                        "
                        @change="changewareHouseSearchFn"
                    >
                    </a-tree-select>
                </a-col>
                <a-col
                    :span="6"
                    :style="{ 'margin-top': btnName !== 'add' && tabChoose === tabTypesNum[0] ? '20px' : 0 }"
                >
                    <span class="form-label">{{ this.tabChoose === 1 ? '经手人' : '作业人' }}</span>
                    <a-input
                        v-model="searchTables.handler_name"
                        type="text"
                        placeholder=""
                        style="width: 70%; height: 24px"
                    />
                ...
                <a-col
                    :span="6"
                    :style="{ 'margin-top': tabChoose !== tabTypesNum[0] && btnName === 'add' ? 0 : '20px' }"
                >
                ...
                <a-col :span="6" style="margin-top: 20px">
                    ...
                    <a-select
                        v-if="c_IsGrn && tabChoose === tabTypesNum[0]"
                        v-model="searchTables.status"
                        allowClear
                        class="form-input form-status-input"
                    >
                        <a-select-option class="input" value="0" key="0">正式</a-select-option>
                        <a-select-option class="input" value="1" key="1">已审核</a-select-option>
                        <a-select-option class="input" value="2" key="2">已入库</a-select-option>
                        <a-select-option class="input" value="4" key="4">作废</a-select-option>
                        <a-select-option class="input" value="5" key="5">红冲</a-select-option>
                    </a-select>
                    <a-select
                        v-if="c_IsGrn && tabChoose === tabTypesNum[1]"
                        v-model="searchTables.status"
                        allowClear
                        class="form-input form-status-input"
                    >
                        <a-select-option class="input" value="0" key="0">待放置</a-select-option>
                        <a-select-option class="input" value="1" key="1">部分放置</a-select-option>
                        <a-select-option class="input" value="2" key="2">作业结束</a-select-option>
                    </a-select>
                    <a-select
                        v-if="!c_IsGrn"
                        v-model="searchTables.status"
                    ...
                    >
                        <a-select-option class="input" value="0" key="0">正式</a-select-option>
                        <a-select-option class="input" value="1" key="1">已审核</a-select-option>
                        <a-select-option class="input" value="2" key="2">已出库</a-select-option>
                        <a-select-option class="input" value="4" key="4">作废</a-select-option>
                        <a-select-option class="input" value="5" key="5">红冲</a-select-option>
                    </a-select>
                </a-col>
                <a-col :span="btnName !== 'add' ? 6 : 12" style="margin-top: 15px">
        ...
        <div
            ...
            :style="{ padding: '8px', background: '#f0f2f5', top: hightSearchObj.isArrow ? '176px' : '56px' }"
        >
            <vxe-table
                ref="xTable"
                size="small"
                :height="scrollYNumber - 1"
                :row-class-name="getRedDashedOrderClassName"
                border
                resizable
                auto-resize
                ...
                align="center"
                :data="godownEntryDatas"
                :loading="godownEntryPagination.loading"
                show-overflow
                show-header-overflow
                showHeader
                :start-index="(godownEntryPagination.current - 1) * godownEntryPagination.pageSize"
            >
                <vxe-table-column
                    v-for="godownEntryColumnItem in godownEntryColumns"
                    :key="godownEntryColumnItem.key"
                    :field="godownEntryColumnItem.dataIndex"
                    :title="godownEntryColumnItem.title"
                    :width="godownEntryColumnItem.width"
                    show-overflow
                ></vxe-table-column>
                <vxe-table-column
                    key="in_time"
                    field="in_time"
                    :title="c_IsGrn ? '入库时间' : '出库时间'"
                    width="15%"
                    show-overflow
                ></vxe-table-column>
            </vxe-table>
            <vxe-pager
                :loading="godownEntryPagination.loading"
                :current-page="godownEntryPagination.current"
                :page-size="godownEntryPagination.pageSize"
                :page-sizes="godownEntryPagination.pageSizes"
                :total="godownEntryPagination.total"
        ...
        <div
            v-else
            class="tab-table-wrap tab-table-overflowy"
            :style="{ background: '#f0f2f5', top: hightSearchObj.isArrow ? '176px' : '56px' }"
            style="padding: 8px"
        >
            <vxe-table
                ref="xTable"
            ...
            >
                <vxe-table-column
                    ...
                    show-overflow
                ></vxe-table-column>
                <vxe-table-column v-if="!c_IsGrn" key="printed" field="printed" title="打印次数">
                    <template v-slot="{ row }">
                        <span
                            :class="{
                                gray: row.printed > 0,
                ...
                <ul class="top-list">
                    ...
                    <li class="item">
                        ...
                        <a-tree-select
                            ...
                            :value="
                                typeof addInfoModalForm.warehouse_inner_sn === 'number'
                                    ? String(addInfoModalForm.warehouse_inner_sn)
                                    : addInfoModalForm.warehouse_inner_sn
                            "
                            :disabled="
                    ...
                    <li class="item">
                        ...
                        <a-tree-select
                            ...
                            class="input"
                            :value="
                                typeof addInfoModalForm.department_inner_sn === 'number'
                                    ? String(addInfoModalForm.department_inner_sn)
                                    : addInfoModalForm.deaprtment_inner_sn
                            "
                        ...
                        >
                            <template v-slot="{ row }">
                                ...
                                <a-input-number
                                    ...
                                    v-model="row.buying_price_converted"
                        ...
                        <vxe-table-column field="produce_date" title="生产日期" width="120">
                            <template v-slot="{ row, seq }">
                                ...
                                <a-date-picker
                                    ...
                                    style="width: 100%"
                                    @change="
                                        (value) => ((row.produce_date = value), updataTable(row, 'overdue_date', seq))
                                    "
                                />
                        ...
                        <vxe-table-column field="overdue_date" title="失效日期" width="120">
                            <template v-slot="{ row, seq }">
                                ...
                                <a-date-picker
                                    ...
                                    @change="(value) => ((row.overdue_date = value), updataTable(row, '', seq))"
                                />
                        ...
                        <vxe-table-column field="mdrf_inner_sn" title="注册证号" width="100">
                            <template v-slot="{ row, seq }">
                                ...
                                <a-tree-select
                                    ...
                                    treeDefaultExpandAll
                                    :value="
                                        typeof row.mdrf_inner_sn === 'number'
                                            ? String(row.mdrf_inner_sn)
                                            : row.mdrf_inner_sn
                                    "
                                    ...
                                    @change="
                                        (value, label) => ((row.mdrf_inner_sn = value), changeMdrfListFn(value, label))
                                    "
                                >
                                </a-tree-select>
                            </template>
                        ...
                        <div class="item-wrap">
                            ...
                            <div class="item">
                                ...
                                <a-tree-select
                                    ...
                                    :value="
                                        typeof addInfoModalForm.handler_inner_sn === 'number'
                                            ? String(addInfoModalForm.handler_inner_sn)
                                            : addInfoModalForm.handler_inner_sn
                                    "
                                    :disabled="isEditDisabledAddInfo || !isIncludedInWarehouseList"
                            ...
                            <div class="item">
                                ...
                                <a-tree-select
                                    ...
                                    :value="
                                        typeof addInfoModalForm.warehouse_operator_inner_sn === 'number'
                                            ? String(addInfoModalForm.warehouse_operator_inner_sn)
                                            : addInfoModalForm.warehouse_operator_inner_sn
                                    "
                                    :disabled="isEditDisabledAddInfo || !isIncludedInWarehouseList"
        ...
        <!-- 出入库退货单的添加产品列表区域结束 -->
        <odo-batch-selecter
            :visible="odoBatchSelecterVisible"
            :orderType="orderTypeData && orderTypeData.value"
            :orderInfo="addInfoModalForm"
            :orderProducts="addInfoDatas"
            :targetProduct="odoBatchSelecterTargetProduct"
            @close="odoBatchSelecterVisible = false"
            @select="onOdoBatchSelect"
        ></odo-batch-selecter>
    </div>
</template>
...
import { DetailTableColumnsGrnAllot, DetailTableColumnsOdoAllot } from './module/transferOrderColumnConfig';

import { CommonGrnListColumns, PurchaseGrnListColumns, CommonOdoListColumns } from './module/GrnAndOdoColumnConfig';
...
export default {
    ...
    data() {
        return {
            ...
            tax_amount_total: 0,
            odoBatchSelecterVisible: false,
            odoBatchSelecterTargetProduct: null
        };
    },
    methods: {
        onOdoBatchSelect(batch) {
            let i = 0;
            while (this.addInfoDatas[i]) {
                if (this.addInfoDatas[i]._XID === this.odoBatchSelecterTargetProduct._XID) {
                    this.addInfoDatas[i].batch_number = batch.batch_number;
                    this.addInfoDatas[i].produce_date = batch.produce_date && moment(batch.produce_date);
                    this.addInfoDatas[i].overdue_date = batch.overdue_date && moment(batch.overdue_date);
                    this.addInfoDatas[i].buying_price = batch.average_price;
                    this.addInfoDatas[i].max_quantity = batch.quantity;
                    if (!this.addInfoDatas[i].quantity) {
                        this.addInfoDatas[i].quantity = batch.quantity;
                    }
                    this.handleTaxUnitPriceChange(this.addInfoDatas[i], 'quantity', i + 1);
                    break;
                }
                i++;
            }
                    ...
                    if (parseFloat(res.code) === 0) {
                        ...
                        list = list.map((f) => {
                            return {
                                ...f,
                                inventoryBannce: 0,
                                inventoryBannceDefault: 0,
                                quantity: Math.max(0, f.quantity)
                            };
                        });
                        record.inventoryBannce = 0;
                        if (this.orderTypeData.value === grnAndOdoOrderType['PURCHASERETURN'].value) {
        ...
        async grnEditAPIFn(status) {
            ...
            this.addInfoDatas.forEach((item) => {
                ...
                item.unit_price = item.tax_unit_price;
                if (item.isOld) {
                    if (
            ...
            this.addInfoDatas.forEach((item, index) => {
                if (item.action) {
                    let obj = {
                        ...
                        'unit_price': item.tax_unit_price,
                        'rate': item.rate / 100,
                    ...
                    if (parseFloat(res.code) === 0) {
                        ...
                        for (let i = 0; i < this.godownEntryDatas.length; i++) {
                            ...
                            if (curObj._XID === item._XID) {
                                ...
                                this.godownEntryDatas.splice(i, 1, curItem);
                                xTable.refreshData();
                                xTable.loadData(this.godownEntryDatas);
                                xTable.setCurrentRow(curItem);
        ...
        changeAddInfoModalWarehouseFn(value, label) {
            this.addInfoModalForm.warehouse_inner_sn = value;
        ...
        changeHandlerNameSnFn(value, label) {
            ...
            this.addInfoModalForm.handler_inner_sn = value;
        ...
        changeWarehouseOperatorNameSnFn(value, label) {
            this.addInfoModalForm.warehouse_operator_inner_sn = value;
                ...
                if (
                    ...
                    _this.$confirm({
                        ...
                        onOk: () => {
                            ...
                            _this.addInfoDatas = [];
                            const xTableAddInfo = _this.$refs.xTableAddInfo;
                            xTableAddInfo.clearData().then(() => {
                                xTableAddInfo.refreshData();
                                _this.dblclickAddProductFn();
                            });
                        },
                        onCancel: () => {
                            _this.isChooseOrgMdTip = false;
        ...
        resetFocusFn(property, rowIndex) {
            ...
            for (let i = 0; i < this.addInfoDatas.length; i++) {
                ...
                if (rowIndex === i) {
                    ...
                    // item.focusDatas = [property === 'product_name', property === 'batch_number', property === 'quantity', property === 'unit', property === 'unit_price', property === 'rate', property === 'produce_date', property === 'overdue_date', property === 'type', property === 'mdrf_inner_sn', property === 'health_care_sn']
                } else {
                    ...
                    if (parseFloat(res.code) === 0) {
                        ...
                        list.forEach((item) => {
                            if (item.status === 'AWAITING') {
                                ...
                                item.price = this.reNumber(item.quantity * item.unit_price);
        ...
        groupBtnFn(btnName) {
            ...
            if (!(btnName === 'add' || btnName === 'bill-lading') && !this.checkEditInfoFormMsgFn()) return;
                ...
                data.forEach((cItem) => {
                    if (item.detail_inner_sn === cItem) {
                        ...
                        item.produce_date = item.produce_date
                            ...
                            : '';
                        item.overdue_date = item.overdue_date
                            ? item.overdue_date === '0000-00-00'
                                ? ''
                                : moment(item.overdue_date)
                            : '';
                        item.name = item.product_name;
        ...
        addInfoModalBotFn(btnName) {
            ...
            if (
                btnName === 'save' &&
                (this.orderTypeData.value === 'SALERETURN' || this.orderTypeData.value === 'PURCHASERETURN')
            ) {
                for (let i = 0, l = this.addInfoDatas.length; i < l; i += 1) {
                    const item = this.addInfoDatas[i];
                    if (item.quantityReturnMax !== undefined && Math.abs(item.quantity) > item.quantityReturnMax) {
                        this.$warning({
                            title: '操作提示',
                            content: `${item.product_name}的退货数量${Math.abs(item.quantity)}不能超过最大可退货数量${
                                item.quantityReturnMax
                            }`,
                            okText: '知道了',
                            centered: true,
                            mask: true
            ...
            switch (btnName) {
                case 'print': {
                    let prDatas = this.addInfoDatas.map((item, index) => {
                        return {
                            ...item,
                            index: index + 1,
                            unit_inner_sn: item.unit_inner_sn.length
                                ? item.unit_inner_sn[item.unit_inner_sn.length - 1]
                                : '',
                            money: new Big(item.quantity).times(item.unit_price).toNumber(), // 金额(含税)
                            produce_date: item.produce_date ? moment(item.produce_date).format(this.format) : '',
                            overdue_date: item.overdue_date ? moment(item.overdue_date).format(this.format) : '',
                            batch_number: item.batch_number.trim()
                        };
        ...
        exportExcel(ref) {
            ...
            const tableData = table.getData().map((item) => {
                ...
                return {
                    ...item,
                    unit: item.unit_name,
                    money: this.$Utils.commafy(new Big(item.quantity).times(item.unit_price).round(2).toNumber()),
                    produce_date: item.produce_date ? moment(item.produce_date).format(this.format) : '',
                    overdue_date: item.overdue_date ? moment(item.overdue_date).format(this.format) : '',
        ...
        changeUnitListFn(value, selectedOptions) {
            ...
            // xTableAddInfo.resetFocusFn('unit_price', seq > 0 ? seq - 1 : 0)
            xTableAddInfo.updateData();
        },
        ...
        refreshTempInsertAddProductFn(tempAddDatas) {
            let xTableAddInfo = this.$refs.xTableAddInfo,
                spliceIndex = this.addInfoDatas.findIndex((item) => !item.product_inner_sn);
            if (spliceIndex > -1) {
                this.addInfoDatas.splice(spliceIndex, 1);
            }
            this.addInfoDatas.push(...tempAddDatas);
            xTableAddInfo.clearCurrentRow();
            xTableAddInfo.updateData();
            const curData = this.addInfoDatas[this.addInfoDatas.length - 1];
            xTableAddInfo.focus();
        ...
        odoBatchNumberFn(record, seq) {
            const orderType = this.orderTypeData.value;
            // 出库单表格明细批号选择事件
            if (!this.addInfoModalForm.warehouse_inner_sn) {
                this.$warning({
                    title: '提示',
            ...
            // 出库单批号选择走新的逻辑
            if (
                orderType === grnAndOdoOrderType.SALEODO.value ||
                orderType === grnAndOdoOrderType.SALERETURN.value ||
                orderType === grnAndOdoOrderType.GIVEODO.value ||
                orderType === grnAndOdoOrderType.ALLOTODO.value ||
                orderType === grnAndOdoOrderType.GRANTODO
            ) {
                this.odoBatchSelecterVisible = true;
                this.odoBatchSelecterTargetProduct = record;
                return;
            }
                ...
                'add': () => {
                    ...
                    if (tempInventoryBannce <= 0 && this.btnName === 'edit') {
                        ...
                        list.forEach((z) => {
                            ...
                            z.batch_locked_quantity = z.quantity;
                    ...
                    for (let i = 0; i < list.length; i++) {
                        ...
                        item.batch_locked_quantity = item.quantity;
            ...
            for (let i = 0; i < this.addInfoDatas.length; i++) {
                ...
                if (item._XID === curObj._XID) {
                    ...
                    this.addInfoDatas.splice(i, 1, {
                        ...
                        batch_number: record.batch_number,
                        max_quantity: record.max_batch_locked_quantity,
        ...
        handleTaxUnitPriceChange(row, actionTypeName, seq) {
            ...
            if (quantity) {
                switch (actionTypeName) {
                    ...
                    // ---- 数量 -------————-------------------------------------------------
                    case 'quantity': {
                        if (row.quantity > Math.abs(row.max_quantity)) {
                            this.$message.warning(`数量不能大于实际可用库存：${row.max_quantity}`);
        ...
        async grnAndOdoAddAPIFn(status) {
            ...
            let params = {
                ...
                warehouse_inner_sn: this.addInfoModalForm.warehouse_inner_sn,
                warehouse_name: warehouseName,
                voucher_inner_sn_c: this.addInfoModalForm.voucher_inner_sn_c
            };
            if (this.addInfoModalForm.department_inner_sn) {
                params.department_inner_sn = this.addInfoModalForm.department_inner_sn;
                params.department_name = this.addInfoModalForm.department_name;
            ...
            this.addInfoDatas.forEach((item, index) => {
                ...
                if (item.product_inner_sn) {
                    ...
                    params.details.push({
                        ...
                        'rate': item.rate / 100,
                        'health_care_sn': item.health_care_sn,
                        // ...temporaryInsertObj  //平台产品退货功能暂时不用 voucher_type_c 不传none
                        'voucher_detail_inner_sn_c': item.voucher_detail_inner_sn_c,
                        'voucher_inner_sn_c': item.voucher_inner_sn_c,
                        'voucher_type_c': item.voucher_type_c
                    });
                ...
                .then((res) => {
                    if (parseFloat(res.code) === 0) {
                        ...
                        if (!this.isClickRedDashedOrder) {
                            let tempInfo = {};
                            if (this.c_IsGrn) {
                                tempInfo.key = `${info.grn_inner_sn}${info.billsType}`;
                            } else {
                                tempInfo.key = `${info.odo_inner_sn}${info.billsType}`;
                                tempInfo.grn_sn = info.odo_sn;
                                tempInfo.in_time = info.out_time;
                            }
                    ...
                    if (parseFloat(res.code) === 0) {
                        ...
                        for (let i = 0; i < this.godownEntryDatas.length; i++) {
                            ...
                            if (curObj._XID === item._XID) {
                                let tempItem = {};
                                if (this.c_IsGrn) {
                                    tempItem.key = `${info.grn_inner_sn}${info.billsType}`;
                                } else {
                                    tempItem.key = `${info.odo_inner_sn}${info.billsType}`;
                                    tempItem.grn_sn = info.odo_sn;
                                    tempItem.in_time = info.out_time;
                                }
                                const curItem = {
                                    ...info,
                                    activeClass: '',
            ...
            if (!isAddReturnDataEmpty) {
                ...
                let oldAddDatas = addReturnDatas.map((item, index) => {
                    ...
                    return {
                        ...item,
                        quantity: -item.quantity,
                        quantityReturnMax: item.quantity,
        ...
        async grnAndOdoTransferRecordDetailAPIFn() {
            let record = this.addInfoModalForm;
            let params = {
                    ...
                    if (parseFloat(res.code) === 0) {
                        ...
                        this.billLadingBotDatasAll = list.map((item) => {
                            ...
                            return {
                                ...
                                price: this.reNumber(item.quantity * item.unit_price)
                            };
                        });
    ...
    computed: {
        ...
        isReturnGoods() {
            ...
            if (
                ...
                let tempDatas = this.addInfoDatas.filter((item) => item.product_inner_sn);
                return tempDatas && tempDatas.length > 0;
            } else {
                return false;
            }
        },
