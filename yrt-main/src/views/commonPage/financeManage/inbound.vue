<template>
    <div class="scoped scoped-white page-finance-inbound">
        ...
        <div style="margin-top: 5px; background: #fff">
            ...
            <list-page-list
                :style="'height:' + scrollYNumber + 'px'"
                ref="list"
                :columns="bizType === 'stock' ? stockTableColumns : columns"
                :list="dataSource"
                :loading="loading"
                :pagination="pagination"
                :footer-method-column="['quantity', 'unit_price', 'amount_of_money']"
                @changePagination="handleChangePagination"
            >
                <template slot="pageLeft">
                    <div class="pricesum_wrapper">
                        <span class="pricesum_label">总金额：</span>
                        <span class="pricesum_value">¥ {{ priceSum }}</span>
                    </div>
                </template>
...
export default {
    ...
    computed: {
        scrollYNumber() {
            ...
            // 281: 199 + 82 - 28
            // 345: 263 + 82 - 28
            return window.innerHeight - (this.bizType === 'stock' ? 220 : 270);
        },
    ...
    methods: {
        ...
        getFilterDataSource(rows = []) {
            ...
            if (this.bizType === 'stock') {
                return rows.map((row) => ({
                    ...row,
                    quantity: transferNum(row.quantity, 1),
        ...
        async getPriceSum() {
            ...
            const formData = this.getFormData(fieldsValue);
            if (this.bizType === 'stock') {
                const { info = 0 } = await getFinanceInventorySumAPI(formData);
                this.priceSum = info === 0 ? 0 : typeof info === 'number' ? info.toFixed(4) : 0;
                return;
            }
            try {
